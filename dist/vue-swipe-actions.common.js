...
// CONCATENATED MODULE: ./src/components/SwipeOut.vue?vue&type=template&id=5806fb9e&
...
/* harmony default export */ var SwipeOutvue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    _createHammer: function _createHammer() {
      ...
      var doubelTab = new hammer_default.a.Tap({
        event: "doubletap",
        taps: 2
      });
    ...
    _swipeListener: function _swipeListener(event) {
      ...
      var newX = this.startLeft + event.deltaX; // attempting to reveal the right actions after revealing the left actions
    ...
    _stopListener: function _stopListener(event) {
      ...
      if (this.startLeft > 0 && event.deltaX <= -this.threshold) return this.closeActions(); // _animateSlide(0, oldLeft);
      // close right actions

      if (this.startLeft < 0 && event.deltaX >= this.threshold) return this.closeActions(); // this._animateSlide(0, oldLeft);

      var currentLeft = this.startLeft + event.deltaX; // reveal left actions
...
component.options.__file = "SwipeOut.vue"
...
var close = ref.close;
return [_vm._t("right",null,{close:close,item:item})]}}])})}),(!_vm.items.length)?[_vm._t("empty",[_vm._v("No results !")])]:_vm._e()],2)}
...
SwipeList_component.options.__file = "SwipeList.vue"
